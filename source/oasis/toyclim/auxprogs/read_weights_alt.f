...
c     write(6,*) rlono(i),rlonow(i),rlonoe(i)
      end do
...
c      read in the igcm land mask
      ...
      close(1)
c     if(mod(itest,100).eq.0)write(6,*)itest
...
c     write(6,*) " eewtes ", ival,jval,sumw,wt(ii,itest)
c     write(6,*) ii,iadress(ii,itest),ivalo,jvalo,ilando(ivalo,jvalo)
      endif
...
c     write(6,*) ii,iadress(ii,itest),ivalo,jvalo,ilando(ivalo,jvalo)
      ...
      if(ival.eq.100 .and. jval.eq.63)then
c     write(6,*) " newtes ", ival,jval,nsea,nland
c     write(6,*) sumw,sumtot
      endif
      if(ival.eq.101 .and. jval.eq.63)then
c     write(6,*) " newtes ", ival,jval,nsea,nland
c     write(6,*) sumw,sumtot
...
c     write(6,*)1,ivalo,jvalo,wt1(1,itest)
      endif
...
!     sumo=sumo+atopao(i,j)
      if(ilando(i,j).ne.0)then
      sumo=sumo+atopao(i,j)*fou(i,j)
c     sumo=sumo+atopao(i,j)
      endif
      end do
      end do
      write(6,*) " SUMFIN = ",sumo
...
!     else
      suma=suma+atopa(i,j)*ofrac(i,j)*fin(i,j)
c     suma=suma+atopa(i,j)*ofrac(i,j)
...
c     sumo=sumo+atopao(i,j)
      ...
      write(6,*) " SUMFIN check = ",sumo

      open(11,file="ofrac_newcalc.igcm3")
      do j=nya,1,-1
